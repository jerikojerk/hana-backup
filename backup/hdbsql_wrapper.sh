...
# read -r -d '' SQL_NEWEST_OBSOLETE_FULL <<QUERY_END
# select MAX(BACKUP_ID)
# from SYS.M_BACKUP_CATALOG
# where SYS_START_TIME <= ADD_DAYS(CURRENT_TIMESTAMP, -<RETENTION> ) AND ENTRY_TYPE_NAME ='complete data backup'
# QUERY_END

# read -r -d '' SQL_COUNT_AVAILABLE_FULL <<QUERY_END
# select COUNT(BACKUP_ID)
# from SYS.M_BACKUP_CATALOG
...
# read -r -d '' SQL_LIST_OBSOLETE_BACKUP <<QUERY_END
# select BACKUP_ID,STATE_NAME,ENTRY_TYPE_NAME,MESSAGE,COMMENT,SOURCE_DATABASE_NAME,SYSTEM_ID,SYS_START_TIME,SYS_END_TIME
# from SYS.M_BACKUP_CATALOG where BACKUP_ID < <BACKUPID>
# QUERY_END

# read -r -d '' SQL_PURGE_BACKUP_COMPLETE_EXACT <<QUERY_END
# BACKUP CATALOG DELETE BACKUP_ID <BACKUPID> COMPLETE
# QUERY_END


# read -r -d '' SQL_PURGE_BACKUP_COMPLETE_BEFORE <<QUERY_END
# BACKUP CATALOG DELETE ALL BEFORE BACKUP_ID <BACKUPID> COMPLETE
# QUERY_END

# read -r -d '' SQL_PURGE_BACKUP_CATALOG_BEFORE <<QUERY_END
# BACKUP CATALOG DELETE ALL BEFORE BACKUP_ID <BACKUPID>
# QUERY_END


##on utilise le BACKUP CATALOG pour effacer.
# preferez hanacleanup svp.
# function hanapurge_hdbsql(){
	# local OPERATOR=$1
	# local WHERE=$2
	# local AGE_DAYS=$3
	# local BACKUP_ID
	...
	# local COUNT_FULL
	# local TMP
	# SQL=${SQL_NEWEST_OBSOLETE_FULL//<RETENTION>/$AGE_DAYS}
	# BACKUP_ID=`hdbsql -U $OPERATOR -x -a "$SQL"`
	# #pour une raison etrange, parfois hana repond ? au lieux de null
	# if [[ "$BACKUP_ID" == "?" || "$BACKUP_ID" == "" ]]
	# then
		# echo "Pas de generation obsolete de backup à purger ($BACKUP_ID)"
		# return
	# else
		# echo "Generation de backup à purger $BACKUP_ID"
	# fi
	# SQL=${SQL_COUNT_AVAILABLE_FULL//<RETENTION>/$AGE_DAYS}
	# COUNT_FULL=`hdbsql -U $OPERATOR -x -a "$SQL"`
	# echo "Nombre de generation de backup à preserver: $COUNT_FULL"
	# if [ $COUNT_FULL -eq 0 ];then
		# echo "Ne veut pas purger, il n'y a pas de full plus recent"
		# logger  -p user.warning -t $LOGGER_TAG "Shall not purge, not enough backup generation ${LOG}"
	# else
		# echo "Va purger les backups suivants:"
		# SQL=${SQL_LIST_OBSOLETE_BACKUP//<BACKUPID>/$BACKUP_ID}
		# hdbsql -U $OPERATOR "$SQL"
		# SQL=${SQL_PURGE_BACKUP_COMPLETE_BEFORE//<BACKUPID>/$BACKUP_ID}
		# hdbsql -U $OPERATOR "$SQL"
		# SQL=${SQL_PURGE_BACKUP_COMPLETE_EXACT//<BACKUPID>/$BACKUP_ID}
		# hdbsql -U $OPERATOR "$SQL"
